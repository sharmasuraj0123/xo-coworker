...
async function request<T>(
  ...
  for (let attempt = 0; attempt <= NETWORK_RETRY_MAX; attempt++) {
    try {
      ...
      //   - ApiError 502/503/504: gateway/upstream errors (e.g. rclone slow,
      //     FastAPI worker overloaded). Connector flows hit these regularly.
      //   - ApiError 500 with empty/non-JSON body: Next.js dev-proxy emits this
      //     when its upstream connection to FastAPI is reset mid-request
      //     (e.g. uvicorn --reload restart). A genuine FastAPI 500 always
      //     returns a JSON body with a `detail` field.
