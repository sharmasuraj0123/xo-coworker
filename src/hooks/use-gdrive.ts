...
import { API, XO_COWORKER_API_BASE } from "@/lib/constants";
 ...
 *
 * Note: bypasses the Next.js dev rewrite when `NEXT_PUBLIC_XO_COWORKER_API_URL`
...
export function useGDriveUpload() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const url = XO_COWORKER_API_BASE ? `${XO_COWORKER_API_BASE}${apiPath}` : apiPath;
